...
class FlaxUpsample2D(nn.Module):
    ...
    def setup(self):
        self.conv = make_conv('1x1',conv3d=self.conv3d,features=self.out_channels,dtype=self.dtype,)
...
class FlaxDownsample2D(nn.Module):
    ...
    def setup(self):
        self.conv = make_conv('down', conv3d=self.conv3d, features=self.out_channels, dtype=self.dtype,)
    @nn.compact
    def __call__(self, hidden_states):
        hidden_states = self.conv(hidden_states)
        hidden_states = nn.with_logical_constraint(
...
class FlaxResnetBlock2D(nn.Module):
    ...
    def setup(self):
        ...
        if use_nin_shortcut:
            self.conv_shortcut = make_conv('1x1', conv3d=self.conv3d, features=self.out_channels,  dtype=self.dtype,)
        out_channels = self.in_channels if self.out_channels is None else self.out_channels

        self.conv1 = make_conv('3x3', conv3d=self.conv3d, features=self.out_channels, dtype=self.dtype,)

        self.time_emb_proj = make_conv('dense',conv3d=self.conv3d, features=self.out_channels, dtype=self.dtype,) 
        
        self.conv2 = make_conv('3x3', conv3d=self.conv3d, features=self.out_channels, dtype=self.dtype,)
