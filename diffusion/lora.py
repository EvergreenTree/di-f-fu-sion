...
def _decision_fn(
    ...
    default = WeightState.FULL if lora_spec.tune_others else WeightState.FROZEN

    # check if the parameter is floating
    if not jnp.issubdtype(param.dtype, jnp.floating):
        return WeightState.FROZEN
...
def _lora_merge(
    ...
    """
    Merge trainable and frozen parameters into a full parameter set.
    """
    trainable = flax.traverse_util.flatten_dict(trainable, sep=PATH_SEP)
    full_params = {}
    full_params.update(frozen)
    ...
    for path in trainable_paths:
        ...
        rank = a.shape[1]
        full_params[path] = jnp.matmul(a, b) * alpha / rank + frozen[path]
...
def lora_init(
    ...
    if lora_spec.disabled:
        ...
        for path, weight in flax.traverse_util.flatten_dict(params, sep=PATH_SEP).items():
            ...
            elif weight_state == WeightState.FROZEN:
                logger.debug(f"Frozen: {path}({weight.dtype})={weight.shape}")
                frozen[path] = weight
            ...
            elif weight_state == WeightState.PSEUDOCONV:
                logger.debug(f"Factorized: {path}({weight.dtype})={weight.shape}")
                trainable[f"{path}{LORA_A_SUFFIX}"] = jax.random.normal(
                    init_rng, (weight.shape[0], rank), dtype=weight.dtype
                ) / jnp.sqrt(weight.shape[0] / 2)
